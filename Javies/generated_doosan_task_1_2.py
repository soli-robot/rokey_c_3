"""
이 코드는 qwen2.5-coder-7B 모델이 다음 System 프롬프트와 User 프롬프트, 그리고 작업반장의 Task 입력을 통해 생성한 1.2 버전 코드입니다.

1.1 버전에서의 문제점이었던 DSR_ROBOT2 모듈이 스크립트 처음에 정의 되어 있는 문제를 다음 수정된 System 프롬프트의 규칙으로 해결하였습니다.
1.1 User 프롬프트와 작업반장의 Task 입력은 동일합니다.
...
============================================================
[DSR_ROBOT2 import 위치 절대 규칙]
============================================================

DSR_ROBOT2는 절대 코드 파일 최상단에서 import하지 않는다.

파일 상단에서 다음과 같은 코드를 절대 작성하지 않는다.
...
이유:
DSR_ROBOT2는 import되는 순간 DR_init.__dsr__node를 사용하여 ROS2 client를 생성한다.
따라서 main()에서 rclpy.init(), rclpy.create_node(), DR_init.__dsr__node = node가 완료되기 전에
DSR_ROBOT2를 import하면 NoneType create_client 에러가 발생한다.

반드시 다음 순서를 지킨다.

1. 코드 상단에서는 import rclpy, import DR_init, import time만 작성한다.
2. 코드 상단에서 DR_init.__dsr__id = ROBOT_ID를 설정한다.
3. 코드 상단에서 DR_init.__dsr__model = ROBOT_MODEL을 설정한다.
4. main()에서 rclpy.init(args=args)를 호출한다.
5. main()에서 node = rclpy.create_node("move_basic", namespace=ROBOT_ID)를 생성한다.
6. main()에서 DR_init.__dsr__node = node를 설정한다.
7. 그 이후 호출되는 initialize_robot(), open_gripper(), close_gripper(), perform_task() 함수 내부에서만 DSR_ROBOT2를 import한다.

DSR_ROBOT2 import는 반드시 함수 내부 local import로만 작성한다.
...
코드 파일 최상단에는 반드시 아래 import만 작성한다.

import rclpy
import DR_init
import time

필요하지 않은 import를 추가하지 않는다.
DSR_ROBOT2 관련 import는 파일 최상단에 절대 작성하지 않는다.
...
반드시 다음 API를 initialize_robot() 함수 내부에서만 import한다.
...
파일 최상단에서도 set_digital_output을 import하지 않는다.
...
반드시 다음 API를 perform_task() 함수 내부에서만 import한다.
...
- wait는 perform_task() 내부에서 import한 DSR_ROBOT2.wait를 사용한다.
...
main() 안에서 DR_init.__dsr__node = node를 설정하기 전에는 initialize_robot(), open_gripper(), close_gripper(), perform_task()를 호출하지 않는다.
...
15. 파일 최상단에서 DSR_ROBOT2를 import
16. DR_init.__dsr__node 설정 전에 DSR_ROBOT2 import
17. DR_init.__dsr__node 설정 전에 initialize_robot(), perform_task(), open_gripper(), close_gripper() 호출
...
- 파일 최상단 import가 import rclpy, import DR_init, import time만으로 구성되어 있는가?
- 파일 최상단에 from DSR_ROBOT2 import ... 가 없는가?
- ROBOT_ID = "dsr01"인가?
- ROBOT_MODEL = "m0609"인가?
- DR_init.__dsr__id = ROBOT_ID가 있는가?
- DR_init.__dsr__model = ROBOT_MODEL가 있는가?
- main() 안에서 rclpy.init(args=args)가 먼저 호출되는가?
- main() 안에서 node = rclpy.create_node("move_basic", namespace=ROBOT_ID)가 있는가?
- main() 안에서 DR_init.__dsr__node = node가 있는가?
- initialize_robot()은 DR_init.__dsr__node = node 이후에 호출되는가?
- DSR_ROBOT2 import는 initialize_robot(), open_gripper(), close_gripper(), perform_task() 함수 내부에만 있는가?
...
입력에 없는 행동은 만들지 마라.
기준 코드 프레임을 유지하라.
DSR_ROBOT2는 절대 파일 최상단에서 import하지 마라.
출력은 Python 코드만 작성하라.
"""
