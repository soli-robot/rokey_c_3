...
4. 생성된 Python 코드를 4060 PC 로컬 파일(4060_local_backup.py)로 백업 저장한다.
5. 생성된 코드를 Python TCP Socket 통신으로 5060 PC에 전송한다.
6. 입력 프롬프트와 생성된 코드를 Firebase Firestore에 로그로 저장한다.
7. 모든 작업이 끝나면 스크립트를 종료한다.

구성 요소:
- SYSTEM_PROMPT:
  생성될 로봇 코드의 구조, 금지 규칙, DSR_ROBOT2 import 위치, RG 그리퍼 제어 방식 등을 정의한다.

- FirebaseLogUploader:
  입력된 작업 시퀀스와 생성된 로봇 코드를 Firebase Firestore에 저장하는 클래스이다.

- send_code_to_5060():
  생성된 Python 코드 문자열을 TCP Socket을 통해 지정된 5060 PC IP로 전송하는 함수이다.

- generate_robot_code():
  Ollama chat API를 호출하여 SYSTEM_PROMPT와 task_sequence를 모델에 전달하고,
  모델 출력을 Python 코드 문자열로 정리하는 함수이다.

- main 실행부:
  5060 PC의 IP를 지정하고, 코드 생성 → 로컬 백업 저장 → 5060 PC로 전송 → Firebase 업로드 순서로 실행한다.

주의사항:
- 이 스크립트는 로봇을 직접 움직이는 코드가 아니라, 로봇 실행용 Python 코드를 생성하고 다른 PC로 전송하는 코드 생성기이다.
- Ollama 서버가 4060 PC에서 실행 중이어야 한다.
- MODEL_NAME에 지정된 모델이 ollama list에 등록되어 있어야 한다.
- BRAIN_5060_IP에는 코드를 수신할 5060 PC의 실제 IP 주소를 입력해야 한다.
- 5060 PC에서는 동일한 포트(기본 9999)로 TCP Socket 수신 서버가 먼저 실행 중이어야 한다.
- Firebase를 사용하려면 resource 폴더 안에 Firebase 인증 JSON 파일이 존재해야 한다.
- 생성된 코드는 실제 로봇에서 실행하기 전에 반드시 문법 검사, 금지어 검사, 버츄얼 검증을 거치는 것이 좋다.
"""
